"""Ultra-simple test — just scrape and exit, no evaluation, no DB, no email."""
...
# Only first 10 fast portals
QUICK_PORTALS = [p for p in PORTALS if p["type"] in ("API", "RSS")][:10]

def run_test():
    scan_start = time.time()
    scan_time = datetime.now().strftime("%Y-%m-%d %H:%M IST")
    logger.info("=" * 70)
    logger.info(f"🚀 Simple Test Starting at {scan_time} ({len(QUICK_PORTALS)} portals)")
    logger.info("=" * 70)
    ...
    logger.info(f"\n{'=' * 70}")
    logger.info(f"✅ Simple Test Complete in {duration:.1f}s")
    logger.info("=" * 70)
    sys.exit(0)
